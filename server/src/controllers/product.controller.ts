...
export async function createProduct(req: Request, res: Response) {
  try {
    const { name, brand, category, description, image_url, skin_type_tags, is_published } = req.body
    if (!name || !brand || !category) {
      res.status(400).json({ error: 'name, brand, and category are required' })
      return
    }
    const product = await prisma.product.create({
      data: { name, brand, category, description, image_url, skin_type_tags: skin_type_tags ?? [], is_published: is_published ?? false }
    })
